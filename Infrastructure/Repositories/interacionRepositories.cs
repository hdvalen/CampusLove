...
namespace InteracionInventario.Repositories
{
    public class InteracionInventario : IRepository<Interacion>
    {
        public async Task<IEnumerable<Interacion>> GetAllAsync()
        {
            List<T> movimientos = new List<interacion>();
            ...
            using (var DataBase = new Db())
            {
                ...
                {
                    interaciones.Add(new InteracionInventario
                    {
                        Id = Convert.ToInt32(reader["id"]),
                        Nombre = Convert.ToDateTime(reader["fecha"]),
                        numInteracion = Convert.ToInt32(reader["tipoMovimiento_id"]),
        ...
        public async Task<bool> UpdateAsync(InteracionInventario)
        {
            using (var DataBase = new DataBase())
            {
                using var command = new MySqlCommand(
        ...
        public async Task<IEnumerable<Mov_Caja>> GetMovimientosByFechaAsync(DateTime fecha)
        {
            ...
            using (var DataBase = new DataBase())
            {
                ...
                while (await reader.ReadAsync())
                {
                    movimientos.Add(new InteracionInventario
                    {
                        Id = Convert.ToInt32(reader["id"]),
                        Nombre = Convert.ToDateTime(reader["fecha"]),
                        TipoMovimientoId = Convert.ToInt32(reader["tipoMovimiento_id"]),
